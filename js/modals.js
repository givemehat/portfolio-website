(function() {
  ...
  function initLoginModal() {
    ...
    if (trigger) {
      trigger.addEventListener('click', function() { openModal('login-overlay'); });
      trigger.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') openModal('login-overlay');
      });
    }
    if (closeBtn) {
      closeBtn.addEventListener('click', function() { closeModal('login-overlay'); });
    }
    if (submitBtn) {
  ...
  function initLeadCapture() {
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', function() { closeModal('lead-overlay'); });
    }
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', function() {
        ...
        var affiliation = document.getElementById('lead-affiliation').value.trim();
        ...
        setTimeout(function() { closeModal('lead-overlay'); }, 1400);
      });
    }
  }
  ...
  function initLeadsViewer() {
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', function() { closeModal('leads-view-overlay'); });
    }
    if (exportBtn) {
      exportBtn.addEventListener('click', function() {
        var blob = new Blob([JSON.stringify(getLeads(), null, 2)], { type: 'application/json' });
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', function() {
        if (confirm('Clear all stored submissions on this browser?')) {
          ...
          renderLeads();
          window.showToast && window.showToast('All submissions cleared.', 'info');
        }
      });
    }
  }
  ...
  function renderLeads() {
    ...
    list.innerHTML = '';
    if (leads.length === 0) {
      list.innerHTML = '<p style="color:var(--text-dim);font-size:13px;padding:10px 0;">No submissions yet on this browser.</p>';
      return;
    }
    leads.slice().reverse().forEach(function(l) {
      var row = document.createElement('div');
      row.style.cssText = 'border:1px solid var(--line);border-radius:10px;padding:12px 14px;font-size:13px;margin-bottom:8px;';
      var when = new Date(l.at).toLocaleString('en-IN');
      row.innerHTML = '<b>' + escapeHTML(l.name) + '</b><br>' +
        '<span style="color:var(--text-dim);">' + escapeHTML(l.location || '—') +
        ' · ' + escapeHTML(l.affiliation || '—') +
        ' · ' + escapeHTML(l.contact || '—') + '</span><br>' +
        '<span style="color:var(--text-dim);font-family:var(--font-mono);font-size:11px;">' + when + '</span>';
  ...
  function initProjectDetails() {
    document.addEventListener('click', function(e) {
      ...
      var techs = [];
      card.querySelectorAll('.tech-badge').forEach(function(t) { techs.push(t.textContent); });
      var status = card.querySelector('.card-status') ? card.querySelector('.card-status').textContent : '';
      ...
      if (bodyEl) {
        bodyEl.innerHTML =
          '<div style="margin-bottom:16px;">' +
            '<span class="tag" style="margin-bottom:12px;display:inline-block;">' + escapeHTML(tag) + '</span>' +
            (status ? ' <span class="card-status card-status--active">' + escapeHTML(status) + '</span>' : '') +
          '</div>' +
          '<p style="color:var(--text-dim);font-size:14px;line-height:1.7;margin-bottom:20px;">' + escapeHTML(desc) + '</p>' +
          (techs.length ? '<div style="margin-bottom:16px;"><strong style="font-size:13px;">Tech Stack</strong><div class="tech-stack" style="margin-top:8px;">' +
            techs.map(function(t) { return '<span class="tech-badge">' + escapeHTML(t) + '</span>'; }).join('') +
          '</div></div>' : '') +
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', function() { closeModal('project-detail-overlay'); });
    }
  }
  ...
  function initToastSystem() {
    window.showToast = function(message, type) {
      ...
      toast.innerHTML = '<span style="font-weight:600;">' + icon + '</span> ' + escapeHTML(message);
