...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tab) {
    ...
    const onMove = (y) => {
      ...
      if (newTop + tab.offsetHeight > window.innerHeight) newTop = window.innerHeight - tab.offsetHeight;
    ...
    tab.addEventListener('mousedown', (e) => { if (e.target.closest('#game-tab')) onStart(e.clientY); });
    document.addEventListener('mousemove', (e) => onMove(e.clientY));
    document.addEventListener('mouseup', () => onEnd());
    
    tab.addEventListener('touchstart', (e) => { if (e.target.closest('#game-tab')) onStart(e.touches[0].clientY); });
  ...
  const INDIAN_NAMES = ['AARAV_07', 'SNEHA_PRO', 'RAHUL_HKS', 'PRIYA_NEO', 'VIKRAM_X', 'ANANYA_99', 'ROHAN_CYBER', 'KAVYA_Z', 'ADITYA_NET', 'ISHA_001', 'KUNAL_BOT', 'NEHA_SYS', 'VARUN_ARC', 'MEGHA_X', 'KARAN_OP', 'SHREYA_V', 'DEV_NULL', 'POOJA_EXE', 'YASH_ROOT', 'RITIKA_AI'];
  ...
  function generateSyntheticData() {
    ...
    if (leaderboards.flyer.length === 0) {
      ...
      let sData = [];
      for(let i=0; i<25; i++) {
        const name = INDIAN_NAMES[Math.floor(Math.random() * INDIAN_NAMES.length)] + Math.floor(Math.random()*99);
        // Flyer Score: max 11 or 12
        fData.push({ alias: name, score: Math.floor(Math.random() * 11) + 2, isUser: false });
      ...
      leaderboards.stroop = sData.sort((a,b) => (b.acc === a.acc) ? a.lat - b.lat : b.acc - a.acc);
  ...
  function getCombinedLeaderboard() {
    ...
    leaderboards.flyer.forEach(p => {
      if(!players[p.alias]) players[p.alias] = { alias: p.alias, flyerScore: 0, stroopScore: 0, isUser: p.isUser };
    ...
    let combined = Object.values(players).map(p => {
      return { alias: p.alias, score: Math.round(p.flyerScore + p.stroopScore), isUser: p.isUser };
    });
  ...
  function showScreen(screenType) {
    ...
    if (screenType === 'SUDOKU') {
      if (sudokuArena) {
        ...
        if (window.SudokuGame && typeof window.SudokuGame.startNewGame === 'function') {
    ...
    if (screenType === 'MINESWEEPER') {
      if (mineArena) {
        ...
        if (window.MinesweeperGame && typeof window.MinesweeperGame.startNewGame === 'function') {
  ...
  function shutdownAllEngines() {
    ...
    if (window.SudokuGame && typeof window.SudokuGame.stopTimer === 'function') {
      window.SudokuGame.stopTimer();
    }
    if (window.MinesweeperGame && typeof window.MinesweeperGame.stopTimer === 'function') {
  ...
  function flyerJump() {
    ...
    for(let i=0; i<5; i++) particles.push(new Particle(flyer.x, flyer.y + flyer.size, '#FFD700', 3, 3));
  }
  ...
  function updateFlyer() {
    ...
    if (flyerFrameCount % 100 === 0) {
      ...
      obstacles.push({ x: flyerCanvas.width, topHeight: topHeight, bottomY: topHeight + gap, width: 40, passed: false });
    }
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
      ...
      if (!obs.passed && flyer.x > obs.x + obs.width) {
        ...
        SFX.score();
        floatingTexts.push(new FloatingText(flyer.x, flyer.y - 30, '+1', '#00FF66'));
      }
      
      // Collision
      if (flyer.x + flyer.size > obs.x && flyer.x - flyer.size < obs.x + obs.width) {
        if (flyer.y - flyer.size < obs.topHeight || flyer.y + flyer.size > obs.bottomY) triggerFlyerCrash();
      }
      
      if (obs.x + obs.width < 0) obstacles.splice(i, 1);
    }

    if (flyer.y + flyer.size > flyerCanvas.height || flyer.y - flyer.size < 0) triggerFlyerCrash();
  ...
  function drawFlyer() {
    ...
    for(let x = -offset; x < flyerCanvas.width; x += 40) { flyerCtx.moveTo(x, 0); flyerCtx.lineTo(x, flyerCanvas.height); }
    for(let y = 0; y < flyerCanvas.height; y += 40) { flyerCtx.moveTo(0, y); flyerCtx.lineTo(flyerCanvas.width, y); }
    flyerCtx.stroke();

    if (flyerActive) {
      flyerCtx.save();
      flyerCtx.translate(flyer.x, flyer.y);
      flyerCtx.rotate(Math.min(Math.PI/4, Math.max(-Math.PI/4, flyer.vy * 0.1)));
    ...
    obstacles.forEach(obs => {
      ...
      flyerCtx.fillRect(obs.x, obs.bottomY, obs.width, flyerCanvas.height - obs.bottomY);
      flyerCtx.strokeRect(obs.x, obs.bottomY, obs.width, flyerCanvas.height - obs.bottomY);
  ...
  function triggerFlyerCrash() {
    ...
    leaderboards.flyer.push({ alias: playerAlias, score: flyerScore, isUser: true });
  ...
  function nextStroopRound() {
    ...
    let options = STROOP_COLORS.map(c => c.name).sort(() => Math.random() - 0.5);
    ...
    if (selectedColor === stroopVisualColor) {
      ...
      if (latency < 1000) {
        ...
        if (currentCombo > 1) {
          ...
          comboTracker.style.animation = 'popIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
        }
      } else {
        currentCombo = 0;
        comboTracker.style.display = 'none';
  ...
  function endStroop() {
    ...
    leaderboards.stroop.push({ alias: playerAlias, acc: stroopCorrect, lat: avgLatency, isUser: true });
    leaderboards.stroop.sort((a,b) => (b.acc === a.acc) ? a.lat - b.lat : b.acc - a.acc);
    localStorage.setItem('lb_stroop', JSON.stringify(leaderboards.stroop));
    ...
    if (stroopCorrect > oldPeakAcc) localStorage.setItem('stroopHighScore', stroopCorrect);
  ...
  // Dashboard Actions
  btnStartFlyer.addEventListener('click', () => requireAlias(() => showScreen('FLYER')));
  btnStartStroop.addEventListener('click', () => requireAlias(() => showScreen('STROOP')));
  
  // Sudoku Integration
  const btnStartSudoku = document.getElementById('btn-start-sudoku');
  if (btnStartSudoku) {
    btnStartSudoku.addEventListener('click', () => requireAlias(() => {
      showScreen('SUDOKU');
  ...
  if (btnStartMinesweeper) {
    btnStartMinesweeper.addEventListener('click', () => requireAlias(() => showScreen('MINESWEEPER')));
  }

  btnViewLeaderboards.addEventListener('click', () => showScreen('LEADERBOARDS'));
  ...
  document.addEventListener('keydown', (e) => {
    if (currentState === 'FLYER' && (e.code === 'Space' || e.code === 'ArrowUp')) {
      e.preventDefault();
      flyerJump();
    }
  });
