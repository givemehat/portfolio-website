...
(function () {
  ...
  const NUM_COLORS = ['', '#3b82f6','#22c55e','#ef4444','#1d4ed8','#991b1b','#0891b2','#1f2937','#6b7280'];
  ...
  function boardEl()   { return $('mine-board'); }
  function arenaEl()   { return $('minesweeper-arena'); }
  ...
  function placeMines(safeR, safeC) {
    ...
    for (let r = 0; r < cfg.rows; r++) {
      for (let c = 0; c < cfg.cols; c++) {
        ...
        let count = 0;
        eachNeighbor(r, c, (nr, nc) => { if (grid[idx(nr,nc)].mine) count++; });
  ...
  function startGame(diff) {
    ...
    const arena = arenaEl();
    if (arena) {
      arena.querySelectorAll('.btn-diff').forEach(b =>
    ...
    if (cell.neighbors === 0) {
      ...
      while (queue.length) {
        ...
        setTimeout(() => {
          ...
          if (el) { el.classList.add('cell-pop'); setTimeout(() => el.classList.remove('cell-pop'), 250); }
        }, delayLocal);
  ...
  function launchConfetti() {
    ...
    canvas.width = arena.offsetWidth; canvas.height = arena.offsetHeight;
    ...
    const pieces = Array.from({length:80}, () => ({
      ...
      rot: Math.random()*360, rs: (Math.random()-.5)*6,
      color: colors[Math.random()*colors.length|0], size: 6+Math.random()*6
    }));
    let frame = 0;
    function tick() {
      ...
      if (++frame < 120) requestAnimationFrame(tick); else canvas.remove();
    }
  ...
  function startTimer() {
    stopTimer();
    timerID = setInterval(() => { if (!isOver) { elapsed++; updateTimer(); } }, 1000);
  }
  ...
  function bindEvents() {
    ...
    // Controls
    $('btn-mine-restart')  ?.addEventListener('click', () => startGame(difficulty));
    $('btn-mine-flag-toggle')?.addEventListener('click', () => setFlagModeUI(!flagMode));
