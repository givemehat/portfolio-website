...
(function () {
  ...
  /* ─── State ─────────────────────────────────────────────── */
  let board = [], solution = [], fixed = [], notes = [];
  let history = [];
  ...
  function generatePuzzle(diff) {
    ...
    shuffleArray(allCells()).slice(0, toRemove).forEach(([r,c]) => puzzle[r][c] = 0);
  ...
  function grid9(fn)  { return Array.from({length:9}, () => Array.from({length:9}, fn)); }
  ...
  function startGame(diff) {
    ...
    setTimeout(() => {
      ...
      document.querySelectorAll('#sudoku-matrix-arena .btn-diff').forEach(b =>
  ...
  function refreshNumpad() {
    ...
    document.querySelectorAll('#sudoku-numpad .btn-num').forEach(btn => {
      ...
      if (!badge) { badge = document.createElement('span'); badge.className = 'num-badge'; btn.appendChild(badge); }
  ...
  /* ─── Notes mode ────────────────────────────────────────── */
  function toggleNotes() { notesMode = !notesMode; setNotesBtn(notesMode); }
  ...
  function startTimer() {
    stopTimer();
    timerID = setInterval(() => { if (!isOver) { elapsed++; renderTimer(); } }, 1000);
  }
  ...
  function endGame(win) {
    ...
    if (nr) nr.style.display = (win && profile[`best_${difficulty}`] === elapsed) ? 'block' : 'none';
  ...
  function launchConfetti() {
    ...
    canvas.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:300;';
    ...
    const colors = ['#00E5FF','#FFD700','#FF0055','#00FF88','#a855f7','#f97316'];
  ...
  function bindEvents() {
    ...
    const arena = $('sudoku-matrix-arena');
    if (arena) {
      arena.querySelectorAll('.btn-diff').forEach(btn =>
    ...
    $('btn-retry-sudoku') ?.addEventListener('click', () => startGame(difficulty));
    ...
    document.addEventListener('keydown', e => {
      ...
      else if (e.key === 'ArrowDown')  { e.preventDefault(); r = Math.min(8, r+1); }
      else if (e.key === 'ArrowLeft')  { e.preventDefault(); c = Math.max(0, c-1); }
      else if (e.key === 'ArrowRight') { e.preventDefault(); c = Math.min(8, c+1); }
      else return;
      selectCell(r, c);
    });
  }
  ...
  function init() {
    ...
    document.querySelectorAll('#sudoku-numpad .btn-num,.btn-num').forEach(btn => {
      if (!btn.dataset.num) btn.dataset.num = btn.textContent.trim().replace(/\D/g,'');
    });
    bindEvents();
