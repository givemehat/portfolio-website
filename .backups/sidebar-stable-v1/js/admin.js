(function () {
  ...
  function initEditableIds() {
    ...
    selectors.forEach(function (sel) {
      document.querySelectorAll(sel).forEach(function (el) {
        ...
        if (
          el.closest("#research-grid") ||
          el.closest("#projects-grid") ||
          el.closest("#publications-grid") ||
          el.closest("#books-grid") ||
          el.closest("#experience-grid")
        )
          return;
  ...
  window.enterAdminMode = function () {
    ...
    window.showToast &&
      window.showToast(
        "Admin mode activated. Everything is now editable.",
        "info",
      );
  };
  ...
  function initAdminBar() {
    ...
    if (saveBtn) {
      saveBtn.addEventListener("click", function () {
        saveContent();
        window.showToast &&
          window.showToast("Content saved to browser.", "success");
      });
    }
  ...
  function addCard(gridId) {
    ...
    window.showToast &&
      window.showToast("New card added. Click to edit.", "success");
  }
  ...
  function saveContent() {
    ...
    [
      ...
      var cards = [];
      grid
        .querySelectorAll(".card:not(.add-card-btn)")
  ...
  function loadSavedContent() {
    var raw;
    try {
      raw = localStorage.getItem(CONTENT_KEY);
    } catch (e) {
      return;
    }
    if (!raw) return;
  ...
  function exportAllData() {
    ...
    try {
      allData.content = JSON.parse(localStorage.getItem(CONTENT_KEY) || "null");
    } catch (e) {}
    try {
      allData.leads = JSON.parse(
        localStorage.getItem("__site_leads__") || "[]",
      );
    } catch (e) {}
    try {
      allData.messages = JSON.parse(
